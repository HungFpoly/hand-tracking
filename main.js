// settings:
const _settings = {
  maskPath: "assets/female_elfs_face.glb",
...
let _autobones = null;
let _isAvatar = false;
let _threeAvatar = null,
  _threeMask = null,
  _threeAvatarSkinnedMesh = null;
...
function main() {
  ...
  init_WebarRocksFace(_canvases.face, _canvases.three, {
    ...
    NNCPath: "./neuralNets/NN_AUTOBONES2_7.json",
    scanSettings: {
      threshold: 0.8,
...
function callbackTrack(detectStates, landmarksStabilized) {
  ...
  if (_autobones) {
    _autobones.update_fromWebARRocks(
...
function toggle_mode() {
  ...
  if (_isAvatar) {
    // append avatar directly to the scene in avatar mode:
    _threeAvatar.visible = true;
    _threeMask.visible = false;
  } else {
    // append avatar to tracker in mask mode
    _threeAvatar.visible = false;
    _threeMask.visible = true;
  }
  document.getElementById("toggleModeButton").innerHTML = _isAvatar
    ? "Mask mode"
    : "Avatar mode";
}
